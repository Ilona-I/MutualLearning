...
function getMarkHtml(markMap, searchParam) {
  let innerHtml = " <div id='markBlock"
      + markMap.get("id")
      + "' style='width: 100%; margin-left:5px;'";
  let currentMark = document.getElementById("defaultCheckMarkList" + markMap.get("id"));
  if (!markMap.get("title").toLowerCase().includes(
          searchParam.toLowerCase())) {
    innerHtml += " hidden ";
  }

  innerHtml +=  "                  <label class=\"custom-control-label\" "
      + " style='width: 90%;'>\n"
  ...
  if (markMap.get("creator") === localStorage.getItem("userLogin")) {
  ...
  innerHtml += "                      <!-- Початок блоку з видаленням мітки -->\n"
      + "                      <div class=\"dropdown\" style=\"margin-left: 5px;\">\n"
      + "                        <button class=\"btn dropdown-toggle\" type=\"button\"\n"
...
function displayMarks() {
  ...
  for (const element of jsonObject) {
    let mark = JSON.parse(JSON.stringify(element));
    let markMap = new Map(Object.entries(mark));
    innerHtml += getMarkHtml(markMap, searchParam);
  }
  document.getElementById("markCheckboxList").innerHTML = innerHtml;
}
